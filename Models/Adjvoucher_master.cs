...
{
    public class Adjvoucher_master
    {
...
cmd.Parameters.AddWithValue("@enteredon", f["enteredon"]);
        ...
        public Boolean SaveMyVoucher(FormCollection f, DataTable udt, DataTable cdt, ref string trid, DateTime finst, DateTime finend,int divisionid,String divname,int secid,int uid)
        {
            ...
            GenHelper g = new GenHelper();
            Int32 myid = g.GetNextId("voucher_master", "id");
            ...
            try
            {
            String Sql = "insert into voucher_master (id,voucherno,vodate,rvnumber,rvdate,sectionid,divid,userid,enteredon,adjtype) ";
            Sql += "values(@id,@voucherno,@vodate,@refvnumber,@refdate,@sectionid,@divid,@userid,@enteredon,@adjtype)";
               
                SqlCommand cmd = new SqlCommand(Sql, con);
                cmd.Parameters.AddWithValue("@id", myid);
                cmd.Parameters.AddWithValue("@voucherno", trid);
                cmd.Parameters.AddWithValue("@vodate", f["vdate"]);
                cmd.Parameters.AddWithValue("@refvnumber", f["refnumber"]);
                cmd.Parameters.AddWithValue("@refdate", f["refdate"]);
                ...
                cmd.Parameters.AddWithValue("@adjtype", f["voctype"]);
                cmd.Transaction = tr;
                int a = cmd.ExecuteNonQuery();

                Int32 newid=g.GetNextId("accounttrans", "id");
                String Trqry = "insert into accounttrans (id,trtype,voucherno,accno,trdate,narration,dramount,cramount,divid,sectionid,userid,enteredon) ";
                Trqry += " values(@id,@trtype,@voucherno,@accno,@trdate,@narration,@dramount,@cramount,@divid,@sectionid,@userid,@enteredon)";
                ...
                String AutoQry = "update autoidgen set adjvoc=adjvoc+1 where divid=" + divisionid.ToString();
